...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	public MinHeap(int maxSize, Ticker ticker) {
		this.arr = new Decreaser[maxSize+1];
		this.size = 0;
		this.ticker = ticker;
	}
	...
	public Decreaser<T> insert(T thing) {
		...
		//
		// FIXME
		//Insert the thing at the end of the array
	    arr[size] = ans;
	    decrease(size);
		ticker.tick();
	...
	void decrease(int loc) {
		...
		//
		//if thing is not parent 
		if (loc != 1 ){
			if (arr[loc].getValue().compareTo(arr[loc/2].getValue()) < 0){
			   moveitem(loc,loc/2);
			   decrease (loc/2);	
			   ticker.tick();
			}else{
	...
	private void heapify(int where) {
		...
		//
		 ...
		 int childl = 2*where;
		 int childr = 2*where + 1;
		 int parent = where;
		 if (childr <= size){	 
	    	if(arr[childl].getValue().compareTo(arr[parent].getValue()) >0 && arr[childr].getValue().compareTo(arr[parent].getValue()) > 0){
	            return;
	  
	        } 
	        if (arr[childl].getValue().compareTo(arr[childr].getValue()) >= 0){ 
	        	moveitem(childr,parent);
	        	heapify(childr);
	        	ticker.tick();
	        	
	        }else{
	        	 moveitem(childl,parent);
	    		 heapify(childl);
		   		ticker.tick();
           		}
	       		}else if (childl <= size){
	        	    	if (arr[childl].getValue().compareTo(arr[where].getValue()) < 0){
	        		    	moveitem(childl,where);
	        				heapify(childl);
	        				ticker.tick();
	        		    }
