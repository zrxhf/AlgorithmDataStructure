...
public class ShortestPaths {
	...
	public void run() {
		...
		while (!pq.isEmpty()){
			VertexAndDist source = pq.extractMin();
			for (Edge i: source.getVertex().edgesFrom()){
			Decreaser<VertexAndDist> Neighbor = map.get(i.to);
			    if (Neighbor.getValue().getDistance() > source.getDistance()+weights.get(i)){
			    Neighbor.decrease(Neighbor.getValue().sameVertexNewDistance(source.getDistance()+weights.get(i)));
			    toEdge.put(i.to, i);
			    }
			}
			}
